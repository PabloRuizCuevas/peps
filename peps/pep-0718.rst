...
is specially usefull in common constructors such as ``list``\, `tuple`` and `dict`` etc.
...
Another example of this is creating a specialized `dict`` for a section of our code where
we want so ensure that keys are strings and values are intergers:

.. code-block:: python

   NameNumberDict = dict[str,int] # specialise the dict

   NameNumberDict(
      one=1,
      two=2,
      three="3" # TypeError: Literal["3"] is not of type int
   )

In spite of the utility of this syntax, when trying to use it with a function,
the code raises, as functions are not subscriptable.
...
additionally, most users would need to learn the difference between ``type` and ```TypeForm``\.
...
In conclusion, the current workarunds feel too complex or verbose, especially compared to syntax that would be consistent with the rest of the language if functions were subscriptable.
...
Generators would benefit immensely from this feature as they can't be properly typed in many scenarios. This occurs because when having generic types the ``send`` and ``yield`` type are often undefined till the generator is used.
...
.. code-block:: python
   ...
   def make_accumulator[T: SupportsAdd]() -> Generator[T, T]:
       """ Adds all elements using the addition operator """
...
Similar examples of this "type Generic specialisation" are also desirable for
functions where their signature is narrowed to different paths in different
parts of the code.

Another use case could be to make the ``.send`` method generic via subscription.
...
This proposal improves the consistency of the type system by allowing a syntax
that already looks and feels natural with the existing syntax for classes.

If accepted this syntax, it will reduce the necessity of ``TypeForm`` in :pep:`747`,  will lower verbosity and cognitive load of safely typed python and
will make built-in types like ``Generators`` and ``AsyncGenerators`` type safe in
situations that previously required more complex workarounds.
