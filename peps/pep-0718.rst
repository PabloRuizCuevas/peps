...
In addition, to the simple usage there are other functionalities without a clear
workaround that wouldn't require a large refactor.

Generators
^^^^^^^^^^

Generators would benefit immensely from this feature as they can't be properly typed in
many scenarios. This occurs because when having generic types the ``send`` and
``yield`` type are often undefined till the generator is used.
...
``TypeForm``,  reduce verbosity and cognitive load of safely typed python and
make built-in types like ``Generators`` and ``AsyncGenerators`` type safe in
